        ...
        <div className="space-y-3">
          {optionKeys.map(key => (
            question.options[key] ? (
              <OptionButton key={key} label={key} text={question.options[key]!} selected={selectedAnswers.includes(key)} result={result} isCorrectOption={question.answer.includes(key)} isMultiple={isMultiple} disabled={hasAnswered} onClick={() => onSelect(key)} />
            ) : null
          ))}
        </div>
        {hasAnswered && (
          <div className={`mt-5 flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium ${result === 'correct' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
            {result === 'correct' ? (<><CheckCircle size={18} /><span>回答正确</span></>) : (<><XCircle size={18} /><span>回答错误，正确答案是 {correctAnswerText}</span></>)}
          </div>
        )}
        {!hasAnswered && selectedAnswers.length > 0 && (
          <button onClick={onSubmit} className="mt-5 w-full py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 active:scale-[0.98] transition-all duration-200">确认答案</button>
        )}
        {!hasAnswered && selectedAnswers.length === 0 && (
          <div className="mt-5 w-full py-3 rounded-xl bg-slate-100 text-slate-400 font-medium text-center">{isMultiple ? '请选择答案（可多选）' : '请选择答案'}</div>
        )}
      </div>
    </div>
  )
